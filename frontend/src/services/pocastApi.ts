...
import type { Podcast, CreatePodcastRequest, PodcastResponse } from "@/types";
...
export const podcastApi = {
	...
	getPodcastsByUser: async (
		userId: number,
	): Promise<{ podcasts: Podcast[]; count: number }> => {
		const response = await api.get<{ podcasts: Podcast[]; count: number }>(
			`/podcasts/user/${userId}`,
		);
		return response.data;
	},

	getPodcastProcessingStatus: async (
		id: number,
	): Promise<{
		podcast_id: number;
		status: string;
